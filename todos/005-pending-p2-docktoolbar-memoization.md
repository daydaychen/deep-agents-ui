...
export const DockToolbar = React.memo(() => {
  ...
  const AuthIcon = useMemo(() => ({
    ...
    read: Eye,
    auto: Zap,
  }[overrideConfig.authMode || "ask"]), [overrideConfig.authMode]);
...
export const DockToolbar = () => {
  ...
  return (
    <DockToolbarView
      ...
      onAuthModeChange={(mode) => setOverrideConfig(prev => ({ ...prev, authMode: mode }))}
      onThinkingToggle={() => setOverrideConfig(prev => ({ ...prev, thinking: !prev.thinking }))}
    />
  );
};
```
