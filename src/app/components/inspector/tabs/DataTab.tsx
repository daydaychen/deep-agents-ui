...
export const DataTab = React.memo(() => {
  ...
  const getRowKey = useCallback((row: any, rowIdx: number) => {
    const rowData = JSON.stringify(row);
    const hash = rowData.split("").reduce((acc, char) => {
      return (acc << 5) - acc + char.charCodeAt(0);
    }, 0);
    return `row-${rowIdx}-${Math.abs(hash)}`;
  }, []);
  ...
  return (
    <div className="flex h-full flex-col p-4 min-w-0">
      ...
      <div className="shrink-0 mb-3 flex items-center justify-between min-w-0">
